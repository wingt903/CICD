...
| Requirement Statement | The organization must enable automated, repeatable deployment and configuration of applications to reduce reliance on manual activities, lower the risk of misconfiguration, and accelerate the secure delivery of services. |
...
| NCSC Cyber Assessment Framework (CAF) | B2.a (Secure configuration of systems) | **NCSC CAF Principles and Guidance baseline (record organization-approved publication date in governance register)** | Outcome identifier format `B2.a` corresponds to CAF Objective B, Principle B2. CAF is published as guidance pages rather than an ISO-style numbered control standard, so organizations should fix and record the approved baseline date for audit consistency. | https://www.ncsc.gov.uk/collection/caf/caf-principles-and-guidance |

## Source-of-Authority Validation Notes

1. The requirement statement itself should be traced to the project/customer governance source (for example policy baseline, contract clause, control catalog, or regulator mapping).
...
4. For ISO alignment, `A.12.1.2` is intentionally mapped to ISO/IEC 27001:2013 control numbering; if your organization transitions to ISO/IEC 27001:2022, maintain a separate crosswalk showing the new Annex A control mapping.
