...
- Scope clarification updated: ServiceNow integration is moved to Phase 2; for Phase 1, manual/governed approval is acceptable and automation starts from the approved GitHub-driven pipeline execution point.

**Status**: **Partially Met** (Phase 1 automation and repeatability are covered; direct ServiceNow-triggered orchestration is deferred to Phase 2)

**Traceability Evidence**
- `architecture/aws-migration-cicd.mmd:57-109`  
  Stages 2 onward model system-driven build, gates, AWS role assumption via OIDC, Ansible build/configuration, AMI registration, and promotion deployment once an approved pipeline run starts.
- `architecture/aws-migration-cicd.mmd:57-109`  
  The delivery flow remains repeatable across environments through the same governed pipeline stages and release controls.
- `architecture/sequence-diagram.mmd:12-40`  
  The sequence diagram still shows the ServiceNow webhook and callback path as the **Phase 2 target state**; in Phase 1, the equivalent automation begins after GitHub approval/dispatch and ends with evidence capture for later ITSM update.
...
**Architect Verdict**
- The architecture partially fulfills App_as_Code_001 for Phase 1: once a governed GitHub pipeline run is approved and started, deployment/configuration execution is automated and repeatable across environments. The remaining gap is the direct ServiceNow trigger/callback integration, which is now deferred to Phase 2.

**Test Scenarios**

| Test Case | Given | When | Then | Pass Criteria |
|---|---|---|---|---|
| TC-001-01: Governed Approval Triggers Automated Pipeline | A valid release/change has passed the Phase 1 GitHub governance process | An approved GitHub Actions workflow is dispatched | The pipeline automatically starts without any manual engineering action after approval | Pipeline run begins within the defined SLA; all post-approval stages execute without human intervention |
...
| TC-001-03: Deployment Evidence Prepared for ITSM Update | A pipeline deployment completes all build and configuration stages | The deployment evidence publication step executes | The AMI ID and Ansible execution evidence are captured in a standard evidence pack for downstream ITSM/CMDB update | The evidence pack contains the AMI ID, execution evidence, and release/change reference needed for manual Phase 1 update or future Phase 2 ServiceNow automation |
...
| TC-003-02: Hardening Validated After Application | Ansible hardening has completed on the builder EC2 | The pipeline triggers an Inspector/ECR scan and SSM inventory capture | The scan confirms all required controls are in place and the result is recorded in the deployment evidence flow | Inspector/ECR scan returns PASS or only findings within the acceptable severity threshold; execution evidence is published in the standard evidence pack for Phase 1 and can be integrated with ServiceNow in Phase 2 |
| TC-003-03: Hardening Failure Detected and Recorded | A hardening step fails to apply a required patch or control during the build stage | The pipeline gate evaluates the hardening outcome | The gate detects the failure, records a FAIL evidence record, and routes the item to the tracked remediation workflow | FAIL gate outcome is recorded in the evidence store with the failing control identified; the release is flagged for remediation follow-up; pipeline does not proceed to image promotion |
...
| `architecture/component-context-diagram.mmd integrations 13-16, 25-26, F1, F2` — gate evidence, pass/fail decisions, remediation routing | Security and quality gate decisions (`SGATE`, `IGATE`) are evaluated automatically by the pipeline. Failures route into the tracked remediation workflow (with ServiceNow integration deferred to Phase 2); passing gates auto-proceed to the next stage. |
...
| `architecture/sequence-diagram.mmd:22-52` | Steps 2–13 model fully automated orchestration: Git checkout, OIDC token acquisition, Ansible controller initialisation, SSM session, EC2 launch, OS hardening, middleware setup, AMI registration, and evidence generation. The sequence diagram's ServiceNow webhook/callback remains a Phase 2 target-state integration; in Phase 1 there is no human action after the governed GitHub dispatch. |
...
| `architecture/full-pipeline-tech-stack.md:row 22` | AWS SSM Inventory reconciles live runtime software and OS state against the governed baseline after build and deployment. The documented CMDB sync remains a Phase 2 integration point, while Phase 1 uses the same evidence for downstream manual ITSM updates. |
...
| `architecture/sequence-diagram.mmd:54-72` | The closed-loop drift detection and auto-remediation sequence runs on a schedule: detect → policy gate → Terraform/config enforce → convergence verify → evidence publication. Where policy permits, no human action is needed to correct post-deployment configuration drift. |
...
| `architecture/sequence-diagram.mmd:50-52` | The build pipeline publishes a normalised 12-field evidence event as part of the deployment evidence step; the Evidence Ingestor Lambda writes a signed, immutable record to S3 Object Lock and indexes it in DynamoDB — creating an auditable log of the automated build and configuration controls. |
...
| `architecture/full-pipeline-tech-stack.md:row 6` | Ansible execution logs are linked to the release/change evidence record, providing per-change evidence that automated configuration controls ran and what the outcome was. ServiceNow linkage is deferred to Phase 2. |
...
| `architecture/aws-migration-cicd.mmd:E1` | The target-state design patches the CMDB/change record with the AMI ID plus Ansible execution evidence. In Phase 1, the same evidence is captured in the standardized release evidence pack for downstream ITSM update. |
...
| `architecture/sequence-diagram.mmd:20-52` | The approved GitHub dispatch → SSM → Ansible → AMI registration → evidence publication flow is automated end-to-end, removing engineer involvement from routine deployment and configuration activities. The ServiceNow webhook/callback remains the Phase 2 target-state integration. |
...
| `architecture/full-pipeline-tech-stack.md:rows 21,22` | SSM Parameter Store segregates environment configuration from code (no manual environment-specific edits at deployment time); SSM Inventory plus standardized evidence output support asset-state reconciliation in Phase 1, with CMDB sync deferred to Phase 2. |

**Architect Verdict**
- App_as_Code_005 is **met for Phase 1 delivery scope**. The architecture eliminates manual verification steps by automating all governed pipeline stages after approval/dispatch (Stages 2–5 in `aws-migration-cicd.mmd`) including automated security gates (SAST, IaC scans, image scans) and system-driven deployment promotion. Post-deployment fixes are prevented by Ansible configuration-as-code baked into immutable golden AMIs before deployment, and a scheduled drift detection and auto-remediation loop (EventBridge → Drift Reconciler → Policy Gate → Enforce Baseline → Convergence Verification) that corrects any post-deployment drift without manual engineering action. Security risk is reduced through consistent, repeatable automated controls applied identically on every pipeline run (tfsec/Checkov, Inspector, GuardDuty, OIDC short-lived credentials, SSM zero-trust tunnel). Every automated control execution publishes a normalised, KMS-signed, tamper-evident evidence record to the compliance evidence store, providing complete auditability. Engineering productivity is improved because build, configuration, scan, deployment, and reconciliation activities are orchestrated automatically, while ServiceNow-triggered orchestration and CMDB callback remain Phase 2 integration items.

**Test Scenarios**

| Test Case | Given | When | Then | Pass Criteria |
|---|---|---|---|---|
| TC-005-01: No Manual Steps Between Approval and Deployment | A release/change has passed the Phase 1 GitHub governance process | The approved workflow dispatches to GitHub Actions | All build, scan, configuration, and deployment steps execute automatically without any human action between approval and evidence publication | Pipeline completes all stages end-to-end without a manual step; the standard evidence pack is generated at completion |
| TC-005-02: Post-Deployment State Matches Approved Baseline | A hardened AMI has been deployed into an environment | The workload enters operation and SSM Inventory reconciliation runs | SSM Inventory and compliance evidence confirm the live runtime state matches the approved Git/Ansible baseline | Convergence check returns PASS; no post-deployment manual configuration fix is required |
...
- App teams may onboard using either **one repository per environment** or **one repository for all environments**; both repository models must consume the same governed templates and mandatory controls.
...
**Status**: **Met** (architecture now explicitly models centralized governed template consumption, mandatory secret-scan and IaC policy controls, and fail-closed override detection with policy-violation errors)
...
| `architecture/sequence-diagram.mmd:43-57` | Promotion loop enforces fail-closed immutable promotion controls with explicit error handling. Any signing-specific references in the upstream sequence remain out of scope for this project unless reintroduced later. |
...
|---|---|
| Centralized governed onboarding model | App teams only select approved hardened templates; application repositories — whether a single multi-environment repo or separate environment-specific repos — reference reusable workflows from a governed template repository using pinned versions, establishing a consistent hardened deployment blueprint for onboarding. |
...
| Evidence | Traceability |
|---|---|
| `architecture/aws-migration-cicd.mmd:61-66` | The Stage 2 gate now evaluates RTCM validation alongside the other quality/security controls, and failures create a tracked remediation outcome with RTCM/policy rejection detail while preventing downstream image build and deployment. |
...
| Fail-closed blocking flow | Wired RTCM decisions into the existing gate/remediation path so unsupported stacks are blocked automatically and routed to the tracked remediation workflow with explicit rejection detail. |
