...
| Scope | Definition |
|---|---|
| **In scope** | Pipeline execution, continuous monitoring and validation outputs, approved runtime baseline, per-check compliance evidence, produced AMI metadata, AWS audit/security logs, and GitHub Actions logs. |
...
| `PIPELINE_RUN` → `COMPLIANCE_EVIDENCE` | 0-or-1 to many | A compliance evidence record may link to a pipeline run when pipeline-triggered; continuous monitoring records can exist without a pipeline run. |
...
- `run_id` (optional pipeline correlation ID; null for continuous monitoring records)
...
- `compliance_status` (health status value used for audit reporting; see value set below)
- `evidence_source` (origin type such as `pipeline_run`, `scheduled_monitor`, `runtime_finding`, `remediation_validation`)
- `checked_at` (evidence timestamp)

### `COMPLIANCE_EVIDENCE.compliance_status` value set

- `COMPLIANT` — check passed and is within approved baseline
- `NON_COMPLIANT` — check failed against approved baseline/control
- `DRIFTED` — previously compliant state deviated from approved baseline
- `PARTIALLY_COMPLIANT` — partial control coverage or mixed sub-check outcomes
- `UNKNOWN` — status cannot be determined due to missing telemetry or incomplete data
- `ERROR` — check execution failed, so no definitive compliance verdict
...
| `COMPLIANCE_EVIDENCE` | ⚠️ Quasi-PII | May include repository/team identifiers and execution metadata from pipeline and monitoring sources | Encryption at rest, least-privilege access, and retention governance |
